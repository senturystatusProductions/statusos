...
(function(){
  ...
  function bind(){
    ...
    document.querySelectorAll('[data-mindset-check]').forEach(el=>el.addEventListener('change',saveMorning));['morningGratitude','mindsetAction'].forEach(id=>document.getElementById(id)?.addEventListener('input',saveMorning));
    ...
    document.getElementById('saveEveningResetBtn')?.addEventListener('click',()=>{const x=current();x.night={wins:document.getElementById('eveningWins')?.value.trim()||'',lesson:document.getElementById('eveningLesson')?.value.trim()||'',evidence:document.getElementById('eveningEvidence')?.value.trim()||'',priority:document.getElementById('tomorrowPriority')?.value.trim()||'',complete:false};x.night.complete=!!(x.night.wins&&x.night.lesson&&x.night.evidence&&x.night.priority);x.night.completedAt=x.night.complete?new Date().toISOString():null;write();const el=document.getElementById('eveningResetStatus');if(el)el.textContent=x.night.complete?'Night review complete. Tomorrow begins with a clear direction.':'Fill in all four reflection fields first.';if(x.night.complete)try{navigator.vibrate?.(20)}catch{}});
