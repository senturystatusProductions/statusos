...
(function(){
  ...
  function render(){
    ...
    const morningDone=document.getElementById('morningTabDone'),nightDone=document.getElementById('nightTabDone');morningDone?.classList.toggle('hidden',!x.morning.complete);nightDone?.classList.toggle('hidden',!x.night.complete);const morningBtn=document.getElementById('completeMorningMindset');if(morningBtn){morningBtn.textContent=x.morning.complete?'Morning Routine Completed':'Complete Morning Routine';morningBtn.classList.toggle('completed',!!x.morning.complete);morningBtn.setAttribute('aria-pressed',String(!!x.morning.complete))}const nightBtn=document.getElementById('saveEveningResetBtn');if(nightBtn){nightBtn.textContent=x.night.complete?'Night Routine Completed':'Complete Night Routine';nightBtn.classList.toggle('completed',!!x.night.complete);nightBtn.setAttribute('aria-pressed',String(!!x.night.complete))};
  ...
  function saveMorningDraft(markComplete=false){const x=current(),previousComplete=!!x.morning.complete;const checks={};document.querySelectorAll('[data-mindset-check]').forEach(el=>checks[el.dataset.mindsetCheck]=el.checked);const next={...x.morning,checks,gratitude:document.getElementById('morningGratitude')?.value.trim()||'',action:document.getElementById('mindsetAction')?.value.trim()||'',complete:previousComplete,completedAt:x.morning.completedAt||null};if(markComplete){next.complete=true;next.completedAt=next.completedAt||new Date().toISOString()}x.morning=next;write();return next.complete}
  function saveMorning(){return saveMorningDraft(false)}
  ...
  function bind(){
    ...
    document.getElementById('completeMorningMindset')?.addEventListener('click',()=>{const ok=saveMorningDraft(true);const el=document.getElementById('morningMindsetStatus');if(el)el.textContent=ok?'Morning routine completed. Now prove it with focused action.':'Morning routine could not be completed. Please try again.';if(ok)try{navigator.vibrate?.(20)}catch{}});
